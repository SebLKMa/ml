...
func main() {
	...
	// The trick to include the bias by just adding another x variable to formula and the matrix
	// i.e.   y = x1 * w1 + x2 * w2 + x3 * w3 + x0 * b
	...
	w := train(X, Y, 100000, float64(0.001))
	...
	for i := 0; i < 5; i++ {
		...
		//fmt.Printf("X row = %#v ", xrow)
		yrow := mat.Row(nil, i, Y)
		//fmt.Printf("=> Y row = %#v\n", yrow)

		newX := mat.NewDense(len(xrow), 1, xrow)
		//matPrint(newX.T())

		p := predict(newX.T(), w)
		fmt.Printf("X[%d] -> %.4f (label: %#v)\n", i, p[0], yrow)
	}
